...
public class DataCollection {
	...
	public void processCombatFrame() {
		...
		for (int i = 0; i < this.Combats.size(); i++) {
			...
			if (next_frame.getStart_time() == 185)
				System.out.println("185");
			
			if (next_frame.isVisited())
				continue;
			if (real_combat == null) {
			...
			if (real_combat.getEnd_time() == next_frame.getStart_time()
					&& real_combat.containSameUnits(real_combat.getRemained_ALL(), next_frame.getInvolved_ALL())) {
				...
				this.Combats.get(i).setVisited(true);
			} else if (real_combat.getEnd_time() == next_frame.getStart_time()
					&& !real_combat.containSameUnits(real_combat.getRemained_ALL(), next_frame.getInvolved_ALL())) {
				continue;
			} else if (real_combat.getEnd_time() == next_frame.getEnd_time()
					&& !real_combat.containSameUnits(real_combat.getRemained_ALL(), next_frame.getInvolved_ALL())) {
				continue;
			} else { // 战斗结束
			...
			//如果combat结束时间正好和游戏结束时间一样，或者combat没结束，但游戏结束了，那就直接保存了
			if(i == this.Combats.size() - 1) {
				real_Combats.add(real_combat);
			}
		}
		...
		for (int i = start; i < end; i++) {
			...
			for (Unit unit : uas.keySet()) {
				if (unit.getID() == u.getID()) {
					...
					if (ua != null && ua.getActionName() == "attack_location")
						return true;
					break;
				}
			}
	...
	// 将数据保存到硬盘
	public void saveJSON(DataCollection data) throws Exception {
		long time = new Date().getTime();
		String filePath = "./dataset/8x8/" + Long.toString(time) + ".json";
	...
	public void saveReadable(DataCollection data) throws IOException {
		long time = new Date().getTime();
		...
		System.out.println("TEXT Data Saved");
	}
	...
	public void toJSON(Writer w) throws Exception {
		w.write(JSON.toJSONString(new CombatList(this.Combats)));
	}
}
